...
(function () {
    ...
    angular.module("Worldview").controller("WorldviewController", WorldviewController);
    ...
    function WorldviewController(WorldviewServices, $cookies) {
                ...
                if (sideLayerModules !== undefined) {
                    ...
                    _getDataExistListByList(tmpList, function (m_timeLineList) {
                        ...
                        _startAnime(timespan, function (err, layerName) {
                            if (self.isLooped) {
                                setTimeout(function () {
                                    _playLatestVideo(sideLayerModules, layerName, -1);
                                }, 5000);
                            }
                        });
                    });
        ...
        function _getDataExistList(layModule, flg, next) {
            ...
            if (!flg) {
                for (var i = 0; i < timeLineListDataAll.length; i++) {
                    if (timeLineListDataAll[i].DataName === layModule.projectName + layModule._id && (timeLineListDataAll[i].DataInfoYear.length > 0 || timeLineListDataAll[i].DataInfoMonth.length > 0 || timeLineListDataAll[i].DataInfoDay.length > 0 || timeLineListDataAll[i].DataInfoMinute.length > 0)) {
        ...
        function _initAnime(JsonData) {
            ...
            for (var i = 0; i < JsonData.length; i++) {
                ...
                var index_z_max = 550;
                if (ItemJson != undefined && m_UrlList != undefined && m_UrlList.length > 0 && m_proid != undefined) {
                    //遍历 存在数据 的URL的列表
