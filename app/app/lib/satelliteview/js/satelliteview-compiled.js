...
ShinetekView.CesiumOpt = {
    ...
    addLayer: function (nameFun, nameLayer, oURL, isBase, WorT) {
        ...
        if (WorT === "WMS") {
            ...
            var layer = new Cesium.ImageryLayer(new Cesium.createTileMapServiceImageryProvider({
                url: oURL,
        ...
        } else if (WorT === "KML") {} else if (WorT === "GEOJSON") {
            console.log("GEOJSON");
...
ShinetekView.OpenlayerOpt = {
    ...
    init: function (url) {
        ShinetekView.map = new ol.Map({
            ...
            controls: ol.control.defaults({
                attribution: false
            }).extend([new ol.control.FullScreen(), //全屏
        ...
        ShinetekView.map.on('change:size', function (e) {
            ...
            if (oScreenBut.className == "ol-full-screen-false") {
                ...
                if (document.getElementsByClassName("productTitle")[0]) {} else {
                    var newProductTitleF = document.createElement("div");
                    newProductTitleF.className = "productTitle";
                    var newProductP = document.createElement("p");
                    newProductTitleF.appendChild(newProductP);
        ...
        var layer = new ol.layer.Tile({
            source: new ol.source.TileImage({
                ...
                tileGrid: new ol.tilegrid.TileGrid({
                    ...
                    resolutions: [0.703125, 0.3515625, 0.17578125, 0.087890625, 0.0439453125, 0.01953125, 0.009765625, 0.0048828125, 0.00244140625 /*, 0.00244140625*/], //设置分辨率
                    /*extent: [-180, -90, 180, 90],*/
                    tileSize: [256, 256]
                }),
                wrapX: false,
    ...
    setScreenTitle: function (nameLayer) {
        var oScreenBut = document.getElementsByClassName("ol-full-screen")[0].getElementsByTagName("button")[0];
        if (oScreenBut.className == "ol-full-screen-false") {} else if (oScreenBut.className == "ol-full-screen-true") {
...
ShinetekView.SatelliteView = {
    ...
    addLayer: function (nameFun, nameLayer, oURL, isBase, WorT) {
        this.getMapFun() == '1' ? ShinetekView.OpenlayerOpt.addLayer(nameFun, nameLayer, oURL, isBase, WorT) : ShinetekView.CesiumOpt.addLayer(nameFun, nameLayer, oURL, isBase, WorT);
    },
    removeLayer: function (nameFun, WorT) {
        this.getMapFun() == '1' ? ShinetekView.OpenlayerOpt.removeLayer(nameFun, WorT) : ShinetekView.CesiumOpt.removeLayer(nameFun, WorT);
    },
    setVisibility: function (nameFun, WorT, isShow) {
        this.getMapFun() == '1' ? ShinetekView.OpenlayerOpt.setVisibility(nameFun, WorT, isShow) : ShinetekView.CesiumOpt.setVisibility(nameFun, WorT, isShow);
    },
    setZIndex: function (nameFun, zIndex) {
        this.getMapFun() == '1' ? ShinetekView.OpenlayerOpt.setZIndex(nameFun, zIndex) : ShinetekView.CesiumOpt.setZIndex(nameFun, zIndex);
    },
    getZIndex: function (nameFun) {
        this.getMapFun() == '1' ? ShinetekView.OpenlayerOpt.getZIndex(nameFun) : ShinetekView.CesiumOpt.getZIndex(nameFun);
    },
    getRe: function () {
        this.getMapFun() == '1' ? ShinetekView.OpenlayerOpt.getRe() : ShinetekView.CesiumOpt.getRe();
    },
    getZoom: function () {
        this.getMapFun() == '1' ? ShinetekView.OpenlayerOpt.getZoom() : ShinetekView.CesiumOpt.getZoom();
    },
    newResolution: function (res) {
        this.getMapFun() == '1' ? ShinetekView.OpenlayerOpt.newResolution(res) : ShinetekView.CesiumOpt.newResolution(res);
    },
    setScreenTitle: function (nameLayer) {
        this.getMapFun() == '1' ? ShinetekView.OpenlayerOpt.setScreenTitle(nameLayer) : ShinetekView.CesiumOpt.setScreenTitle(nameLayer);
    },
    oGetStatus: function () {
        if (this.getMapFun() == '1') {
...
//# sourceMappingURL=satelliteview-compiled.js.map